...
(*  expose  — the lead-in of a top-level ExposeDecl in a .bni (§16.5.2);
...
TopLevelDecl  = [ Annotation ] ( TypeDecl
              ...
              | InterfaceDecl
              | ImplDecl
              | ExposeDecl ) ;      (* ExposeDecl is permitted only in a .bni; §16.5.2 *)

(* `expose "P"` re-exports package P's entire exported surface as part of THIS
   package's exported surface (§16.5.2); it is permitted only in a .bni
   interface file.  `expose` is a CONTEXTUAL keyword — an ordinary identifier
   except as the lead-in of a top-level declaration. *)
...
BuiltinCall   = "make" "(" Type ")"
              ...
              | "__c_entry" "(" Expression ")" ;               (* C-callable function pointer as *uint8 — §16.9; Draft *)
