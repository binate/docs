...
- generic instantiations and synthesized destructor/copy helpers (emitted
  into every consuming module);
...
overrides it); TU-local blobs (string bytes, RTTI name/field blobs) are
**local** and invisible to resolution.
...
storage in `bss`. The abstract sections map to:
...
arm32 hard-float objects carry `.ARM.attributes` declaring v7/VFPv3/
VFP-register argument passing (soft-float objects omit the section).
...
- A word-sized **data symbol reference** (a vtable slot, a handle field, a
  TypeInfo pointer) is an absolute-address relocation
  (`R_<ARCH>_ABS64/ABS32` / `*_RELOC_UNSIGNED`) over zeroed placeholder
  bytes.
...
- **PIE policy**: Binate links position-independent **executables**; a
  PC-relative reference to a *defined* symbol (weak included) binds to the
  image's own copy (ELF `PC32`), and only undefined-at-assembly targets use
  `PLT32`. A shared-object target would need the preemptible-symbol rules
  and is out of scope.
- **C globals** (`__c_global`): on PIE targets the address is loaded
  GOT-indirect (x86-64 `GOTPCREL`; aarch64 ADRP+LDR GOT pair); on the
  static/non-PIE arm32 targets it is materialized with a MOVW/MOVT absolute
  pair and **no GOT**.
...
from each package's satisfaction-graph node and built at startup — it is
deliberately **not** part of the TypeInfo record.
...
`bn_entry` builds the interface-satisfaction registry, runs package
initialization in dependency order, then calls `main.main`
(language spec §17).
